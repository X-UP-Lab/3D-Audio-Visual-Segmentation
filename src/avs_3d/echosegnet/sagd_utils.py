...
def mask_inverse(xyz, viewpoint_camera, sam_mask):
    ...
    point_mask = torch.full((point_image.shape[0],), -1).to(depth.device)
...
def ensemble(multiview_masks, filtering_threshold=0.5):
    """Combine multiple per-view boolean masks into a single voting label.

    `multiview_masks` should be a list of tensors shape (N, 1). Function returns (vote_labels, indices)
    """
    multiview_masks = torch.cat(multiview_masks, dim=1)
    count_0 = (multiview_masks == 0).sum(dim=1)
    count_1 = (multiview_masks == 1).sum(dim=1)
    vote_labels = torch.where((count_1 / (count_0 + count_1 + 1e-7)) > filtering_threshold, 1, 0)
    labels_mask = (vote_labels == 1)
...
def gaussian_decomp(gaussians, viewpoint_camera, input_mask, indices_mask):
    ...
    conv2d = conv2d_matrix(gaussians, viewpoint_camera, indices_mask, device='cuda:0')
    height = viewpoint_camera.image_height
    width = viewpoint_camera.image_width
